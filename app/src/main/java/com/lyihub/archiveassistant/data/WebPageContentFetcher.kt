...
class DefaultWebPageContentFetcher(
    ...
    override suspend fun fetch(originalUrl: String, fetchUrl: String): WebPageContentFetchResult =
        withContext(Dispatchers.IO) {
            val response = runCatching { transport.fetch(fetchUrl) }.getOrElse { error ->
            ...
            WebPageContentFetchResult.Success(extractWebPageContent(originalUrl, fetchUrl, resolvedUrl, html, contentType))
        }
}
...
class OkHttpWebPageTransport(
    private val client: OkHttpClient = defaultWebPageOkHttpClient(),
) : WebPageTransport {
    override suspend fun fetch(url: String): WebPageResponse = withContext(Dispatchers.IO) {
...
private fun defaultWebPageOkHttpClient(): OkHttpClient = OkHttpClient.Builder()
