...
object AiEnginePresetPreferences {
    ...
    private val presetFields = listOf(
    ...
    fun presetFromCurrentSettings(name: String, settings: AiEngineSettings, rawApiKey: String): AiEnginePreset =
        AiEnginePreset(
            name = name,
    ...
    private fun encodePresetArray(presets: List<AiEnginePreset>): String {
        ...
        val entries = presets.joinToString(",\n") { preset ->
    ...
    private fun parsePreset(obj: Map<String, String>): AiEnginePreset {
        val engineType = obj["engineType"]
            ?.let { name -> AiEngineType.entries.firstOrNull { it.name == name } }
