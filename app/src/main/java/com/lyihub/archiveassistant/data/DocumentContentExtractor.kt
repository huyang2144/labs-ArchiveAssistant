...
class DefaultDocumentContentExtractor(
    ...
    override suspend fun extract(
        ...
        format: DocumentFormat,
        fileName: String?,
    ): DocumentContentExtractionResult = withContext(Dispatchers.IO) {
        val resolvedName = fileName?.trim()?.takeIf { it.isNotBlank() } ?: resolveDisplayName(context, uri)
        val detectedFormat = if (format != DocumentFormat.UNKNOWN) format else detectFormatFromName(resolvedName)
        val content = runCatching {
            ...
            return@withContext DocumentContentExtractionResult.Failure(mapDocumentExtractionError(error))
        }
    ...
    ZipInputStream(input).use { zipInput ->
        while (true) {
            ...
            if (entry.name == "word/document.xml") {
                ...
                val text = extractDocxXmlText(xmlBytes.copyOfRange(0, minOf(xmlBytes.size, MAX_DOCX_XML_BYTES)))
                return boundedContent(fileName, DocumentFormat.DOCX, text, xmlBytes.size > MAX_DOCX_XML_BYTES)
            }
        }
    }
...
private fun boundedContent(
    ...
    return ExtractedDocumentContent(
        ...
        isTruncated = upstreamTruncated || normalized.length > bounded.length || bounded.length > MAX_REQUEST_CONTEXT_CHARS,
    )
}
...
private fun decodeText(bytes: ByteArray): String {
    if (bytes.startsWith(0xEF, 0xBB, 0xBF)) return bytes.drop(3).toByteArray().toString(Charsets.UTF_8)
...
private fun normalizeWhitespace(value: String): String =
    value.replace("\u0000", " ").lineSequence().joinToString("\n") { line ->
        line.trim().replace(INLINE_WHITESPACE_REGEX, " ")
    }.trim()

private fun detectFormatFromName(fileName: String): DocumentFormat = when (fileName.substringAfterLast('.', "").lowercase()) {
...
private fun mapDocumentExtractionError(error: Throwable): String = when (error) {
